...
// The background plane with the image map
plane {
    z, 1 // A plane normal to the Z-axis, 1 unit from the origin
    pigment {
        image_map {
            png "hubble-nebula-monochrome.png" // Specify the image file name and type
            once map_type 0
        }
        // Add a scaling and translation to fit the image correctly
        scale <W, W, 1> // Adjust scale as needed
        translate <-W/2, -W/2, 0> // Push the plane further back
    }
    finish {
        ambient 1
    }
    // ensure no shadows are cast on the background plane
    hollow on
    no_shadow
}
